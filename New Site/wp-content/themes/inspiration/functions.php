...
add_theme_support( 'genesis-style-selector', array( 'discovery-silver' => 'Silver', 'discovery-green' => 'Green', 'discovery-red' => 'Red', 'discovery-brown' => 'Brown', 'discovery-blue' => 'Blue', 'discovery-black' => 'Black' ) );

add_action( 'genesis_meta', 'discovery_add_viewport_meta_tag' );
function discovery_add_viewport_meta_tag() {
    echo '<meta name="viewport" content="width=device-width, initial-scale=1.0"/>';
}


add_filter('body_class','discovery_color_body_class');
if(isset($_GET['background'])) add_filter('body_class','discovery_background_body_class');

function discovery_color_body_class($classes) {
	$color = "silver";
	if(isset($_GET['color'])) $color = $_GET['color'];

	$classes[] = 'discovery-'.$color;
	return $classes;
}

function discovery_background_body_class($classes) {
	$background = $_GET['background'];
	$classes[] = 'discovery-'.$background;
	return $classes;
}


/** Child theme (do not remove) */
define( 'CHILD_THEME_NAME', 'discovery Theme' );
define( 'CHILD_THEME_URL', 'http://www.studiopress.com/themes/discovery' );
...
add_action('genesis_before_header','discovery_add_demo_nav');
function discovery_add_demo_nav() {
...
add_action( 'genesis_after_header', 'discovery_top_search' );
/**
 * Add top search widget area on Genesis after header hook
 *
 */
function discovery_top_search() {
...
add_action( 'genesis_after', 'discovery_disclaimer' );
/**
 * Add disclaimer widget area on Genesis after hook
 *
 */
function discovery_disclaimer() {
...
add_filter( 'discovery_property_details', 'discovery_property_details_filter' );
/**
 * Filter the property details array.
 *
 */
function discovery_property_details_filter( $details ) {
...
add_filter( 'discovery_featured_listings_widget_loop', 'discovery_featured_listings_widget_loop_filter' );
/**
 * Filter the loop output of the discovery Featured Listings Widget.
 *
 */
function discovery_featured_listings_widget_loop_filter( $loop ) {
...
genesis_register_sidebar( array(
	'id'			=> 'top-search',
	'name'			=> __( 'Top Search', 'discovery' ),
	'description'	=> __( 'This is the top search section.', 'discovery' ),
) );
genesis_register_sidebar( array(
	'id'			=> 'slider',
	'name'			=> __( 'Slider', 'discovery' ),
	'description'	=> __( 'This is the slider section.', 'discovery' ),
) );
genesis_register_sidebar( array(
	'id'			=> 'property-search',
	'name'			=> __( 'Property Search', 'discovery' ),
	'description'	=> __( 'This is the property search section.', 'discovery' ),
) );
genesis_register_sidebar( array(
	'id'			=> 'welcome',
	'name'			=> __( 'Welcome', 'discovery' ),
	'description'	=> __( 'This is the welcome section.', 'discovery' ),
) );
genesis_register_sidebar( array(
	'id'			=> 'properties',
	'name'			=> __( 'Properties', 'discovery' ),
	'description'	=> __( 'This is the properties section.', 'discovery' ),
) );
genesis_register_sidebar( array(
	'id'			=> 'communities',
	'name'			=> __( 'Communities', 'discovery' ),
	'description'	=> __( 'This is the communities section.', 'discovery' ),
) );
genesis_register_sidebar( array(
	'id'			=> 'featured-bottom-left',
	'name'			=> __( 'Featured Bottom Left', 'discovery' ),
	'description'	=> __( 'This is the featured bottom left section.', 'discovery' ),
) );
genesis_register_sidebar( array(
	'id'			=> 'featured-bottom-right',
	'name'			=> __( 'Featured Bottom Right', 'discovery' ),
	'description'	=> __( 'This is the featured bottom right section.', 'discovery' ),
) );
genesis_register_sidebar( array(
	'id'			=> 'disclaimer',
	'name'			=> __( 'Disclaimer', 'discovery' ),
	'description'	=> __( 'This is the disclaimer section.', 'discovery' ),
) );
...
function custom_footer_creds_text() {
    ...
    $creds .=  date( 'Y' );
    $creds .=  ' &middot; <a href="http://mywebsite.com">Company Name</a> &middot; Built on the <a href="http://www.studiopress.com/themes/genesis" title="Genesis Framework">Genesis Framework</a>';
    $creds .=  '</p></div>';
    return $creds;
}